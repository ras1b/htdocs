...
class XenForo_ControllerPublic_Thread extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionShowPosts()
	{
		...
		if (empty($posts))
		{
			return $this->responseError(new XenForo_Phrase('no_posts_matching_criteria_specified_were_found'), 404);
		}
	...
	public function actionPollReset()
	{
		...
		if ($action == 'remove')
		{
			...
			if (XenForo_Visitor::getUserId() != $thread['user_id'])
			{
				XenForo_Model_Log::logModeratorAction('thread', $thread, 'poll_delete');
			}
		}
		else if ($action == 'reset')
		{
			...
			if (XenForo_Visitor::getUserId() != $thread['user_id'])
			{
				XenForo_Model_Log::logModeratorAction('thread', $thread, 'poll_reset');
			}
		}
