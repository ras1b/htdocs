...
class XenForo_ControllerPublic_Search extends XenForo_ControllerPublic_Abstract
{
	...
	public function actionSearch()
	{
		...
		if ($origKeywords && !$input['keywords'])
		{
			return $this->responseError(new XenForo_Phrase('your_search_not_completed_contains_only_censored_words'));
		}
		...
		if (!$search)
		{
			...
			if ($origKeywords !== $input['keywords'])
			{
				$searcher->warning(new XenForo_Phrase('search_contained_censored_been_ignored'), 'censor');
			}
