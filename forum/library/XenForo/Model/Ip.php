...
class XenForo_Model_Ip extends XenForo_Model
{
	...
	public function logCookieLoginIfNeeded($userId, $ipAddress = null)
	{
		if (!$userId)
		{
			return 0;
		...
		$binaryIp = XenForo_Helper_Ip::getBinaryIp(null, $ipAddress);
		if (!$binaryIp)
		{
			return 0;
		}
		...
		$cutOff = XenForo_Application::$time - 6*3600;
		$recentLog = $this->_getDb()->fetchOne("
			SELECT 1
			FROM xf_ip
			WHERE user_id = ?
				AND log_date > ?
				AND ip = ?
			LIMIT 1
		", array($userId, $cutOff, $binaryIp));
		if ($recentLog)
		{
			return 0;
		}

		return $this->logIp($userId, 'user', $userId, 'cookie_login', $ipAddress);
	}
