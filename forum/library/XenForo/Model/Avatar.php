...
class XenForo_Model_Avatar extends XenForo_Model
{
	...
	public function applyAvatar($userId, $fileName, $imageType = false, $width = false, $height = false, $permissions = false)
	{
		...
		$maxDimensions = reset(self::$_sizes);
		$sizeCode = key(self::$_sizes);
		$largeSizeCode = $sizeCode;
		...
		foreach (self::$_sizes AS $sizeCode => $maxDimensions)
		{
			if ($sizeCode == $largeSizeCode)
			{
				continue;
			}
	...
	public function recropAvatar($userId, $x, $y)
	{
		...
		$largeMaxDimensions = reset(self::$_sizes);
		$largeSizeCode = key(self::$_sizes);
