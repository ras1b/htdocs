...
class XenForo_Helper_Facebook
{
	...
	public static function getAccessToken($url, $code)
	{
		...
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.10/oauth/access_token');
	...
	public static function getUserInfo($accessToken, $path = 'me')
	{
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.10/' . $path);
			$client->setParameterGet('fields', 'id,name,email,birthday,gender,website,location,link');
			if ($accessToken)
			{
	...
	public static function getUserPicture($accessToken, $size = 'large')
	{
		try
		{
			$client = XenForo_Helper_Http::getClient('https://graph.facebook.com/v2.10/me/picture?type=' . $size);
			$client->setParameterGet('access_token', $accessToken);
	...
	public static function getFacebookRequestUrl($redirectUri, $appId = null, $state = null)
	{
		...
		return 'https://www.facebook.com/v2.10/dialog/oauth?client_id=' . $appId
			. '&scope=' . $perms
			. '&state=' . $state
