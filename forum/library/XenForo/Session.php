...
class XenForo_Session
{
			...
			if ($userCookie)
			{
				...
				if ($userId = $userModel->loginUserByRememberCookie($userCookie))
				{
					...
					if (XenForo_Application::getConfig()->enableTfa
						&& $user['use_tfa']
						&& $tfaModel->userRequiresTfa($user['user_id'])
					)
					{
					...
					if ($allowLogin)
					{
						...
						XenForo_Model::create('XenForo_Model_Ip')->logCookieLoginIfNeeded($userId);
					}
				}
				else
				{
	...
	public static function startAdminSession(Zend_Controller_Request_Http $request = null)
	{
		...
		if ($userId = $session->get('user_id'))
		{
			...
			{
				$languageId = XenForo_Application::getDb()->fetchOne(
					'SELECT admin_language_id FROM xf_admin WHERE user_id = ?', $userId
				);
			}
		}

		$visitor = XenForo_Visitor::setup($session->get('user_id'), array('languageId' => intval($languageId)));
