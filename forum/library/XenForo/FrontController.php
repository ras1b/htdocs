...
class XenForo_FrontController
{
	...
	*/
	public function run()
	{
		...
		{
			if (preg_match('#(^|/)sitemap\.xml$#', $requestPaths['requestUri']))
			{
				$renderer = new XenForo_SitemapRenderer();
				$renderer->outputSitemap();
				return;
			}
			else if (preg_match('#(^|/)sitemap-(\d+)\.xml$#', $requestPaths['requestUri'], $match))
			{
				$renderer = new XenForo_SitemapRenderer();
				$renderer->outputSitemap($match[2]);
				return;
			}
		}
