...
class XenForo_DataWriter_Option extends XenForo_DataWriter
{
	...
	*/
	...
	{
		if (($this->isChanged('default_value') || $this->isChanged('data_type')) && $this->get('data_type') == 'array')
		{
			if (strlen($this->get('default_value')) && !is_array(XenForo_Helper_Php::safeUnserialize($this->get('default_value'))))
			{
				$id = htmlspecialchars($this->get('option_id'));
				$this->error("Developer: default_value is not a valid array ($id)", 'default_value');
			}
		}
		...
		{
			if ($this->getOption(self::OPTION_VALIDATE_VALUE))
			{
			...
			else if ($this->get('data_type') == 'array')
			{
				if (strlen($this->get('option_value')) && !is_array(XenForo_Helper_Php::safeUnserialize($this->get('option_value'))))
				{
					$id = htmlspecialchars($this->get('option_id'));
					$this->error("Developer: option_value is not a valid array ($id)", $this->get('option_id'));
				}
			}
		}
